...
Prepares verified backup evidence before mutation and performs authorized restore operations.

## Consumed Contract Types

`ExecutionContext`, `RecoveryPlan`, `RollbackPlan`

## Produced Contract Types

`BackupResult`, `RollbackResult`, `DiagnosticReference`

## Operations

- `prepareBackup(context: ExecutionContext, plan: RecoveryPlan) -> BackupResult`
- `executeRollback(context: ExecutionContext, plan: RollbackPlan) -> RollbackResult`
...
persistence for backup records and approved mutation for rollback restoration only.
