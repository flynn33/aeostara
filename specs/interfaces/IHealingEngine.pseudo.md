...
Orchestrates the fixed Aeostara healing lifecycle without implementing platform I/O or mutation mechanics.

## Consumed Contract Types

`ObservedSystemState`, `DesiredSystemIntent`, `SemanticProjectionSpec`, `PolicyBundle`, `ExecutionContext`

## Produced Contract Types

`DryRunResult`, `HealResult`, `RecoveryPlan`, `VerificationPlan`, `DiagnosticChain`, `AuditChain`

## Operations

- `dryRun(observedState: ObservedSystemState, desiredIntent: DesiredSystemIntent, projectionSpec: SemanticProjectionSpec, policyBundle: PolicyBundle) -> DryRunResult`
- `heal(observedState: ObservedSystemState, desiredIntent: DesiredSystemIntent, projectionSpec: SemanticProjectionSpec, policyBundle: PolicyBundle, executionContext: ExecutionContext) -> HealResult`
...
dryRun is none; heal performs only approved lifecycle orchestration and delegates mutation to adapters..
