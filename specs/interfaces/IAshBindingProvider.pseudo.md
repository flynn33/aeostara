...
- `evaluateAdmissibility(state: AshState) -> AdmissibilityResult`
- `diagnoseState(state: AshState) -> StateValidityDiagnostic`
- `classifyState(diagnostic: StateValidityDiagnostic) -> SystemStateClass`
- `applyCodewordTransformation(transformation: CodewordTransformation) -> TransitionResult`
- `resolveRealmIdentity(state: AshState) -> RealmIdentity`
- `resolveTransition(state: AshState, definition: TransitionDefinition) -> TransitionResult`
- `generateTopology(state: AshState) -> TopologyPlan`
- `evaluateAxioms(state: AshState) -> sequence<AxiomDiagnostic>`
- `generateMaterializationPlan(state: AshState) -> GenerationPlan`
- `planArtifactEmission(plan: GenerationPlan) -> ArtifactEmissionPlan`
