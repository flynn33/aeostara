...
Applies platform-neutral JSON semantics to in-memory documents; it does not authorize or persist platform mutation.

## Consumed Contract Types

`JsonDocumentReference`, `JsonPointer`, `JsonMutationOperation`

## Produced Contract Types

`JsonDocumentReference`, `SurfaceDifference`, `DiagnosticReference`

## Operations

- `canonicalize(documentReference: JsonDocumentReference) -> JsonDocumentReference`
- `resolvePointer(documentReference: JsonDocumentReference, pointer: JsonPointer) -> (resolvedPointer: JsonPointer, value: JSON_VALUE | ABSENT)`
- `generateSurfaceDifferences(observed: JsonDocumentReference, desired: JsonDocumentReference) -> sequence<SurfaceDifference>`
- `validateMutationPreconditions(documentReference: JsonDocumentReference, operation: JsonMutationOperation) -> DiagnosticReference`
