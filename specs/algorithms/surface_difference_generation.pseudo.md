...
This active algorithm defines deterministic Aeostara base-design behavior for `surface_difference_generation.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.

- JSON Pointer is the only address form; dot-containing keys are ordinary keys. Missing and JSON null remain distinct. Canonical JSON sorts object keys lexicographically, preserves array order, emits deterministic strings and numbers, and hashes with lowercase SHA-256. Surface differences are evidence only and never directly authorize recovery steps.
...
```text
FUNCTION run_surface_difference_generation(input, lifecycleContext) -> contractOutput
