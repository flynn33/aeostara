...
This active algorithm defines deterministic Aeostara base-design behavior for `mutation_precondition_check.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.

- JSON Pointer is the only address form; dot-containing keys are ordinary keys. Missing and JSON null remain distinct. Canonical JSON sorts object keys lexicographically, preserves array order, emits deterministic strings and numbers, and hashes with lowercase SHA-256.
...
```text
FUNCTION run_mutation_precondition_check(input, lifecycleContext) -> contractOutput
