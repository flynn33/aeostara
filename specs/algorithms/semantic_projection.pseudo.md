...
This active algorithm defines deterministic Aeostara base-design behavior for `semantic_projection.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.

- Projection emits exactly b0, b1, b2, b3, b4, b5, b6, b7, b8 in that order. Missing evidence is BLOCKED, ambiguous evidence is AMBIGUOUS, malformed evidence is FAILED, and projection performs no mutation.
...
- The primary projection output is `MappingResult`; it contains exactly nine `SemanticDimensionBinding` records ordered b0 through b8.
...
```text
FUNCTION run_semantic_projection(input, lifecycleContext) -> contractOutput
