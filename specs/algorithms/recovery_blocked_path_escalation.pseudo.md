...
This active algorithm defines deterministic Aeostara base-design behavior for `recovery_blocked_path_escalation.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.

- Recovery begins after diagnosis, classification, and RecoveryCategory selection. NO_ACTION has no executable steps; mutation plans require policy and backup; blocked paths escalate through rollback, fallback, containment, and terminal safe halt as specified.
- Escalation is monotonic: policy, backup, execution, verification, rollback, fallback, and containment failures may move only toward a safer terminal state and cannot resume normal execution without verified recovery evidence.
...
```text
FUNCTION run_recovery_blocked_path_escalation(input, lifecycleContext) -> contractOutput
