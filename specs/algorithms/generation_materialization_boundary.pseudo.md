...
This active algorithm defines deterministic Aeostara base-design behavior for `generation_materialization_boundary.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.

- The algorithm consumes fixed ASH baseline bindings only: 512 F2^9 states, the 16 canonical codewords, XOR transformations, admissibility, diagnostics, realm, transition, topology, axiom, and materialization boundaries without local semantic extension.
...
```text
FUNCTION run_generation_materialization_boundary(input, lifecycleContext) -> contractOutput
