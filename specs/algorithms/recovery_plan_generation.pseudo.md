...
This active algorithm defines deterministic Aeostara base-design behavior for `recovery_plan_generation.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.
...
- Escalation is monotonic: a blocked or failed recovery path may move only toward rollback, fallback, containment, or safe halt and cannot return to normal execution without verified evidence.
...
```text
FUNCTION run_recovery_plan_generation(input, lifecycleContext) -> contractOutput
