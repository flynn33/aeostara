...
This active algorithm defines deterministic Aeostara base-design behavior for `safe_halt_semantics.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.
...
```text
FUNCTION run_safe_halt_semantics(input, lifecycleContext) -> contractOutput
