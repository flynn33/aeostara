...
This active algorithm defines deterministic Aeostara base-design behavior for `mapping_failure_handling.pseudo.md`. Authority flows from the ASH Pattern System baseline through the frozen Aeostara contracts, ASH bindings, conformance fixtures, and lifecycle documents.
...
```text
FUNCTION run_mapping_failure_handling(input, lifecycleContext) -> contractOutput
